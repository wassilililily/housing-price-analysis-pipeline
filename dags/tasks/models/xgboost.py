...
def xgboost_model(data_file_path, test_file_path):
    ...
    try:
        ...
        # === Feature Engineering (same as local final version) ===
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            plt.title("XGBoost: Actual vs Predicted (Log)")
            plt.grid(True)
            plt.tight_layout()
            pdf.savefig()
            plt.close()
            ...
            plt.axis('off')
            plt.text(0.1, 0.5, f"""XGBoost Final Model Metrics
...
""", fontsize=14, verticalalignment='center', fontweight='bold')
        ...
        logging.info(f"Final XGBoost complete | RMSE: {rmse:.2f}, R²: {r2:.4f}, R²_log: {r2_log:.4f}")
        logging.info(f"Final report saved to: {pdf_path}")

    except Exception as e:
        logging.error(f"XGBoost DAG task failed: {e}")
        raise
